...
namespace AutoRest.CSharp.Output.Models.Types
{
    internal sealed class ModelTypeProviderFields : IReadOnlyCollection<FieldDeclaration>
    {
        ...
        public ModelTypeProviderFields(IReadOnlyList<InputModelProperty> properties, CSharpType modelType, InputModelTypeUsage inputModelUsage, InputDictionaryType? additionalPropertiesType, bool isStruct, bool isPropertyBag, TypeFactory typeFactory, ModelTypeMapping? sourceTypeMapping)
        {
            ...
            {
                var originalFieldName = BuilderHelpers.DisambiguateName(modelType, inputModelProperty.Name.ToCleanName());
                ...
                var field = existingMember is not null
                    ? CreateFieldFromExisting(existingMember, serialization, propertyType, GetPropertyInitializationValue(propertyType, inputModelProperty), typeFactory, inputModelProperty.Type, inputModelProperty.IsRequired, optionalViaNullability)
            ...
            if (additionalPropertiesType is not null)
            {
                ...
                var additionalPropertiesField = new FieldDeclaration($"Additional Properties", accessModifiers | ReadOnly, type, declaration, null, false, Serialization.SerializationFormat.Default, true);
            ...
            if (sourceTypeMapping is not null)
            {
                foreach (var serializationMapping in sourceTypeMapping.GetSerializationMembers())
                {
                    ...
                    var field = CreateFieldFromExisting(serializationMapping.ExistingMember, serializationMapping, originalType, null, typeFactory, null, false, false);
        ...
        private static ValidationType GetParameterValidation(FieldDeclaration field, InputModelProperty inputModelProperty)
        {
            ...
            // this is validation for a model's property, there is never a case we need to assert not null or empty
        ...
        private static FieldDeclaration CreateField(string fieldName, CSharpType originalType, InputModelTypeUsage usage, InputModelProperty inputModelProperty, bool isStruct, bool isPropertyBag, bool optionalViaNullability)
        {
            ...
            return new FieldDeclaration(
                ...
                SerializationBuilder.GetSerializationFormat(inputModelProperty.Type, originalType),
        ...
        private static bool PropertyIsReadOnly(InputModelProperty property, InputModelTypeUsage usage, CSharpType type, bool isStruct, bool isPropertyBag)
        {
            ...
            // output only models do not need setters
            if (!usage.HasFlag(InputModelTypeUsage.Input))
            {
                return true;
            }

            // a property will not have setter when it is required literal type
        ...
        private static FieldDeclaration CreateFieldFromExisting(ISymbol existingMember, SourcePropertySerializationMapping? serialization, CSharpType originalType, ValueExpression? initializationValue, TypeFactory typeFactory, InputType? propertyType, bool isRequired, bool optionalViaNullability)
        {
            ...
            else if (existingMember is IPropertySymbol { SetMethod: { } setMethod })
            {
                var modifiers = GetAccessModifiers(setMethod);
                if (modifiers != Public)
            ...
            return new FieldDeclaration(
                ...
                propertyType is not null ? SerializationBuilder.GetSerializationFormat(propertyType, originalType) : Serialization.SerializationFormat.Default,
        ...
        {
            if (inputModelProperty.ConstantValue is { } constant && !propertyType.IsNullable)
            {
                return new ConstantExpression(BuilderHelpers.ParseConstant(constant.Value, propertyType));
            }
